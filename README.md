...
> **R:** Al escribir la biblioteca con #include, se están agregando además de la línea de código que hicimos nosotros, todas las que están en ese archivo de biblioteca.
...
> **R:** Porque el preprocesador borra todos los comentarios, no llegan al archivo .i
...
> **R:** 2194:    printf("=== Laboratorio de Compilacion en C (v%s) ===\n\n", "1.0");
...
> **R:**
> gcc -E programa.c | grep "Iniciando" -> (no devuelve nada)
> gcc -E -DDEBUG programa.c | grep "Iniciando" -> printf("[DEBUG] %s\n", ("Iniciando main"));
...
> **R:** Dicen de qué archivo viene cada parte del código y en qué línea estaba originalmente. El printf viene de stdio.h que está en N:/Facultad/Programas/mingw64/x86_64-w64-mingw32/include/stdio.h
...
> **R:** Guardan el estado de la pila antes de entrar a la función y preparan el espacio para que pueda ejecutarse.
...
0000000000000000 r .xdata
                 ...
                 U area_circulo
                 U factorial
000000000000014c T imprimir_separador
...
> **R:** Porque area_circulo está definida en matematica.c, no en programa.c. programa.o sabe que existe pero no tiene su código, por eso aparece como U.
...
> **R:** Sigue siendo U porque esa función viene de una biblioteca que se carga cuando el programa arranca, no antes.
...
> **R:** La macro CUADRADO(x) se reemplaza por el código antes de compilar, en el preprocesamiento, como un copia y pega. La función real como sumar existe en el ejecutable y se llama cuando corre el programa. La macro no chequea tipos, la función sí.
...
> **R:** T es para las funciones, viven en la sección text del archivo objeto. D es para variables globales que tienen un valor inicial, viven en la sección data.
...
> **R:**
        ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7fff0ea30000)
        KERNEL32.DLL => /c/Windows/System32/KERNEL32.DLL (0x7fff0df50000)
        KERNELBASE.dll => /c/Windows/System32/KERNELBASE.dll (0x7fff0c300000)
        ucrtbase.dll => /c/Windows/System32/ucrtbase.dll (0x7fff0c840000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque gcc la incluye automáticamente siempre, no hace falta pedirla explícitamente.
...
a. ¿Cuántas veces se evalúa `n++`? ¿Por qué? 2 veces, porque la macro reemplaza CUADRADO(n++) por ((n++) * (n++)), entonces n++ aparece dos veces en el código.
b. ¿Cuál sería el resultado? ¿Es el esperado? Da 12 (3 * 4), no es lo esperado. Si hacés el cuadrado de 3 esperabas 9, pero como n++ se evalúa dos veces el resultado cambia.
c. ¿Cómo se resolvería este problema usando una función real en lugar de una macro? int cuadrado(int x) { return x * x; }

### E2 — Provocar y leer errores del compilador

a. **Error léxico:** en `programa.c`, escribí una cadena sin cerrar: `printf("hola`. Intentá compilar con `gcc -S programa.c`. ¿En qué etapa falla y qué dice el mensaje de error? Falla en la etapa de compilación. El error dice missing terminating " character en la línea 58, o sea detectó que abriste una cadena y nunca la cerraste. Los errores de las líneas siguientes son consecuencia del primero.

b. **Error sintáctico:** quitá un `;` al final de una declaración de variable. ¿Qué reporta el compilador? ¿Menciona la línea correcta? Falla en compilación también, y sí menciona la línea correcta. Dice expected ';' before 'int'. El segundo error que aparece ('i' undeclared) es consecuencia del primero.

c. **Error semántico:** cambiá la llamada `sumar(3, 4)` por `sumar(3, 4, 5)`. ¿En qué etapa falla? ¿Por qué es semántico y no sintáctico? Falla en compilación. Dice too many arguments to function 'sumar'; expected 2, have 3. Es semántico y no sintáctico porque la llamada está bien escrita como forma, el problema es el significado — estás pasando más argumentos de los que acepta la función.

d. **Error de enlazado:** comentá toda la implementación de `factorial` en `matematica.c` y recompilá solo el objeto (`gcc -c matematica.c -o matematica.o`). ¿Falla? Ahora intentá enlazar. ¿Cuándo falla y cuál es el mensaje exacto? Al compilar solo el objeto no falla, porque el prototipo está declarado y el compilador lo da por válido. Falla cuando intentás enlazar, con el mensaje undefined reference to 'factorial' — el enlazador buscó la implementación en todos los objetos y no la encontró en ninguno.
...
c. Ejecutar `ldd programa_static` (Linux). ¿Qué diferencia hay respecto a `ldd programa`? En Linux el estático mostraría not a dynamic executable porque tiene todo adentro, y el dinámico mostraría las bibliotecas que necesita. En Windows no se nota la diferencia.
